...
- [~] **M1b — web half DONE (2026-07-19, local main, not pushed); mobile half pending owner deps.** Done: banner render on Standings rows + ThroneSheet raters + rating-submit moment (each user's equipped banner threaded through the standings/ratings payloads via `users.equipped`, no migration), and constant-time webhook-auth compare (`node:crypto` `timingSafeEqual`). Green (279 tests). **Still pending (owner-dep gated):** mobile RevenueCat SDK purchase UI (`react-native-purchases`) in `apps/mobile`.
