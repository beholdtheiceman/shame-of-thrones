...
- [x] **M1b — CODE-COMPLETE (2026-07-19, local main, not pushed).** Web: banner render on Standings rows + ThroneSheet raters + rating-submit moment (each user's equipped banner threaded through the standings/ratings payloads via `users.equipped`, no migration) + constant-time webhook-auth compare (`node:crypto` `timingSafeEqual`). Mobile: `react-native-purchases@10.4.3` Treasury screen (buy/equip/restore) + purchases service + RevenueCat init (appUserID = `users.id`, now exposed via `mePayload.profile.id`); fail-safe no-ops when unconfigured. Gate: 280 web+core tests green, web build clean, mobile `tsc` clean. **Not verifiable here** — the mobile purchase flow needs the owner's RevenueCat project + App Store/Play products + an EAS device build to exercise. Mobile banner *render* parity (showing other users' banners on mobile Standings) is a deferred follow-up.
