...
- [ ] Notifications: opt-in categories, rate-limited to 1 territory push/day — "Fief Contested," "Banner has fallen," season start/rally, freshness quests — **DESIGN COMMITTED, not yet built** (`docs/superpowers/specs/2026-07-13-phase3-notifications-design.md`). Plan: one persisted `notifications` table feeding an in-app inbox (ship first) + a deferred Web Push transport (blocked on owner-set VAPID env vars); generation at `submitRating` (contested/flip) + lazy season_start; 24h dedupe. **Implementation deferred to a fresh session** — the last Phase 3 item.
