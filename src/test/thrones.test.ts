...
describe("thrones", () => {
  ...
  it("the adder cannot confirm their own throne", async () => {
    const user = await makeUser();
    const throne = await addThrone(user, { name: "Selfie Privy", lat: 40.75, lng: -73.99, category: "park", amenities: AMENITIES, publicAccessAttested: true });
  ...
  it("a second user confirms: verified, 25 to adder's house, 3 to confirmer", async () => {
    ...
    const throne = await addThrone(adder, { name: "Real Privy", lat: 40.75, lng: -73.99, category: "transit", amenities: AMENITIES, publicAccessAttested: true });
  ...
  it("confirming twice 409s", async () => {
    ...
    const throne = await addThrone(adder, { name: "Popular Privy", lat: 40.75, lng: -73.99, category: "cafe", amenities: AMENITIES, publicAccessAttested: true });
